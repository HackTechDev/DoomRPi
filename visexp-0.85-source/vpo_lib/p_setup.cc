...
// or NULL on success.
//
const char * P_SetupLevel ( const char *lumpname )
{
    ...
    {
        sprintf(error_msg, "No such map in wad: %s", lumpname);
        return error_msg;
    }

    if (lumpinfo[base].is_hexen)
    {
        sprintf(error_msg, "Hexen map format is not supported yet.");
        return error_msg;
    }
    ...
    {
        sprintf(error_msg, "Missing nodes for: %s", lumpname);
        return error_msg;
    }
...
///    P_LoadBlockMap (base + ML_BLOCKMAP);
    ...
    P_GroupLines ();
///    P_LoadReject (base + ML_REJECT);

    P_LoadThings (base + ML_THINGS);
